...
from .agent_browser_profile import AgentBrowserProfile
...
class AgentBrowserSession(BrowserSession):
    ...
    browser_use_tools: BrowserUseTools | None = Field(default=None, exclude=True)
    ...
    async def connect(self, cdp_url: str | None = None) -> Self:
        ...
        # Store the original HTTP URL for later use
        original_http_url = self.cdp_url
        ...
        if not self.cdp_url.startswith('ws'):
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    self.logger.debug(f"Fetching WebSocket URL from: {url}")
                    version_info = await client.get(url, headers=headers, timeout=10.0)
                    self.logger.debug(f"Version info response status: {version_info.status_code}")
                    if version_info.status_code == 200:
                        version_data = version_info.json()
                        self.logger.debug(f"Version info data: {version_data}")
                        web_socket_url = version_data.get('webSocketDebuggerUrl')
                        if web_socket_url:
                            self.browser_profile.cdp_url = web_socket_url
                            self.logger.debug(f"Converted HTTP URL to WebSocket URL: {web_socket_url}")
                        else:
                            self.logger.error(f"No webSocketDebuggerUrl found in version info: {version_data}")
                            raise RuntimeError(f'No webSocketDebuggerUrl found in version info')
                    else:
                        self.logger.error(f"Failed to get version info, status code: {version_info.status_code}, response: {version_info.text}")
                        raise RuntimeError(f'Failed to get version info, status code: {version_info.status_code}')
                except Exception as e:
                    self.logger.error(f"Failed to fetch WebSocket URL from {url}: {e}")
                    raise RuntimeError(f'Failed to fetch WebSocket URL: {e}') from e
    ...
    async def connect_agent(self, target_id: str) -> Self:
        ...
        if not hasattr(self, '_cdp_client_root') or self._cdp_client_root is None:
            if self.main_browser_session:
                # Use the main browser session's CDP client
                self._cdp_client_root = self.main_browser_session._cdp_client_root
            else:
                await self._safe_connect(max_attempts=3)

        # Always connect to the target, regardless of whether it's already in the pool
        self.logger.info(f"Agent {self.id}: Assigned target {target_id}")
        
        # Use the main browser session's CDP client if available
        cdp_client = self.main_browser_session._cdp_client_root if self.main_browser_session else self._cdp_client_root
        
        # Create CDP session for the target with retry logic
        max_attempts = 3
        last_exception = None
        for attempt in range(max_attempts):
            try:
                self.agent_focus = await CDPSession.for_target(cdp_client, target_id, new_socket=False,
                                                               cdp_url=self.cdp_url)
                break
            except Exception as e:
                last_exception = e
                self.logger.warning(f"Failed to create CDP session for target {target_id} (attempt {attempt + 1}/{max_attempts}): {e}")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(2 ** attempt)  # Exponential backoff
        else:
            raise RuntimeError(f"Failed to create CDP session for target {target_id} after {max_attempts} attempts: {last_exception}")
        
        # Activate the target and run debugger
        try:
            ...
            self.logger.warning(f"Failed to activate target {target_id}: {e}")
        
        # Add to session pool
        self._cdp_session_pool[target_id] = self.agent_focus
        return self

    async def _safe_connect(self, max_attempts: int = 5):
        """Safely connect with retry logic."""
        last_exception = None
        
        for attempt in range(max_attempts):
            try:
                await self.connect()
                self.logger.debug(f"Successfully connected agent on attempt {attempt + 1}")
                return
            except Exception as e:
                last_exception = e
                self.logger.warning(f"Failed to connect agent (attempt {attempt + 1}/{max_attempts}): {e}")
                if attempt < max_attempts - 1:  # Don't sleep on the last attempt
                    await asyncio.sleep(2 ** attempt)  # Exponential backoff
        
        raise RuntimeError(f"Failed to connect agent after {max_attempts} attempts: {last_exception}")
    ...
    async def attach_all_watchdogs(self) -> None:
        ...
        from app.browser.watchdog.action_watchdog import CustomActionWatchdog
        from app.browser.watchdog.dom_watchdog import CustomDOMWatchdog
    ...
    async def navigate_to_url(self, url: str, new_tab: bool = False) -> Optional[str]:
        ...
        if not url or not isinstance(url, str):
            self.logger.error(f'Invalid URL provided for navigation: {url}')
            return None
            
        # Check if URL is valid
        if not url.startswith(('http://', 'https://', 'file://', 'about:', 'data:')):
            # If it doesn't start with a valid protocol, assume it's a web URL
    ...
    async def get_target_id_from_tab_id(self, tab_id: str) -> str:
        """Get target ID from tab ID."""
        # For now, we'll assume tab_id is the same as target_id
        # In a more sophisticated implementation, we might maintain a mapping
        return tab_id

    async def get_target_id_from_url(self, url: str) -> str:
        """Get target ID from URL by finding a tab with that URL."""
        tabs = await self.get_tabs()
        for tab in tabs:
            if tab.url == url:
                return tab.target_id
        # If no tab found with exact URL, create a new one
        cdp_client = self.main_browser_session._cdp_client_root if self.main_browser_session else self._cdp_client_root
        result = await cdp_client.send.Target.createTarget(params={'url': url})
    ...
    async def get_most_recently_opened_target_id(self) -> str:
        ...
        if tabs:
            ...
            cdp_client = self.main_browser_session._cdp_client_root if self.main_browser_session else self._cdp_client_root
            result = await cdp_client.send.Target.createTarget(params={'url': 'about:blank'})
            return result['targetId']

    async def stop(self):
        """Stop the browser session and clean up all resources."""
        try:
            # Disconnect all agent-specific sessions
            await self.disconnect_agent()
            
            # Stop the main browser session if it exists and is not shared
            if self.main_browser_session and hasattr(self, '_is_shared_session') and not self._is_shared_session:
                try:
                    await self.main_browser_session.stop()
                except Exception as e:
                    self.logger.warning(f"Error stopping main browser session: {e}")
            ...
            if hasattr(self, '_cdp_client_root') and self._cdp_client_root:
                try:
                    ...
                    self.logger.warning(f"Error stopping CDP client: {e}")
            ...
            if hasattr(self, '_watchdogs_attached') and self._watchdogs_attached:
                try:
                    # Stop all watchdogs
                    watchdog_attrs = [
                        '_downloads_watchdog', '_local_browser_watchdog', '_security_watchdog',
                        '_popups_watchdog', '_permissions_watchdog', '_default_action_watchdog',
                        '_screenshot_watchdog', '_dom_watchdog'
                    ]
                    
                    for attr_name in watchdog_attrs:
                        if hasattr(self, attr_name):
                            watchdog = getattr(self, attr_name)
                            if watchdog and hasattr(watchdog, 'detach_from_session'):
                                try:
                                    watchdog.detach_from_session()
                                except Exception as e:
                                    self.logger.warning(f"Error detaching watchdog {attr_name}: {e}")
                except Exception as e:
                    self.logger.warning(f"Error cleaning up watchdogs: {e}")
            
            self.logger.info("Browser session stopped successfully")
        except Exception as e:
            self.logger.error(f"Error during browser session stop: {e}")
            raise
